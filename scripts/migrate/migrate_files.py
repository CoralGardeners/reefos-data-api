...
# nurseries
nurseries = qf.get_docs(qf.query_nurseries({}))
...
def fix_mc_site_images(mc_sites):
    cnt = 1
...
fix_mc_site_images(mc_sites)

mon_events = cg_monitorings + cc_monitorings

# %%

def fix_frag_event_images(mon_events):
    events = [ev for ev in mon_events if len(ev[1]['eventData']['images']) != 0]
    missing_images = []
    cnt = 1
    for event_id, event in events:
        if (len(missing_images) + 1) % 100 == 0:
            pd.DataFrame(missing_images).to_csv('missing_images.csv', index=False)
        print(f"Updating {cnt} of {len(events)}, missing {len(missing_images)}")
        loc = event['location']
        ...
        for image in images:
            ...
            for img_type, img_name in image.items():
                ...
                if len(name_parts) > 2 and name_parts[1] == 'images':
                    ...
                    blob = bucket.blob(full_name)
                    if not blob.exists():
                        missing_images.append({'orgID': loc['orgID'],
        ...
        print(new_images)
        if not debug:
            event['eventData']['images'] = new_images
            uf.update_document('_events', event_id, event)
        cnt += 1
    pd.DataFrame(missing_images).to_csv('missing_images.csv', index=False)

fix_frag_event_images(mon_events)
